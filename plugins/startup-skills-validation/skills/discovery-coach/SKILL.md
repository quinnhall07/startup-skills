...
- `references/bias-sentinel.md` — especially confirmation bias and polite customer.
- `references/mom-test-principles.md` — the question-design rules.
- `references/evidence-weighting-matrix.md` — for DEBRIEF classification.
- `references/false-signal-detection.md` — the 7 patterns.
- `references/email-templates.md` — for outreach to set up interviews.
- `references/external-resources.md` — Fitzpatrick, Friedman, Dinnr.
...
2. **Generate a custom 12–18 question interview script** built from `references/mom-test-principles.md`. Required shape:
...
3. **Outreach to set up the interview.** From `references/email-templates.md`, draft 1–3 outreach messages (warm and cold variants) targeting the ICP. Customize the credibility line and value-prop sentence to the specific recipient.
...
2. **Classify every substantive statement against `references/evidence-weighting-matrix.md`.** Per statement: category, weight (0.0–1.0), one-line reasoning. Example:
...
3. **Apply `references/false-signal-detection.md`.** Run all 7 patterns over the new entries AND the existing log. Especially: Enthusiasm Trap, Network Halo, Polite Customer, Feature Request Mirage. If any fire, name the pattern and the offending entries.
...
8. **Bias sentinel pass per `references/bias-sentinel.md`.** Almost always: confirmation bias (founder minimizing negative quotes), polite customer (founder overweighting positive quotes). Name them.
...
Per `references/tone-and-stance.md`. In PREPARE: senior partner coaching, brisk. In DEBRIEF: forensic. The founder will lobby for higher weights — refuse politely but firmly. Diplomatic dishonesty here costs the founder months.
